    ...
    if REQ_CHANNEL1 and not await is_requested_one(client, message):
        btn = [[
            InlineKeyboardButton(
                "𝗥𝗲𝗾𝘂𝗲𝘀𝘁 𝘁𝗼 𝗝𝗼𝗶𝗻 ⚓️ 1", url=client.req_link1)
        ]]
        ...
        try:
            if REQ_CHANNEL2 and not await is_requested_two(client, message):
                btn.append(
                      [
                    InlineKeyboardButton(
                        "𝗥𝗲𝗾𝘂𝗲𝘀𝘁 𝘁𝗼 𝗝𝗼𝗶𝗻 ⚓️ 2", url=client.req_link2)
                      ]
                )
        ...
        sh = await client.send_message(
            ...
            text="**♦️ 𝗥𝗘𝗔𝗗 𝗧𝗛𝗜𝗦 𝗜𝗡𝗦𝗧𝗥𝗨𝗖𝗧𝗜𝗢𝗡 ♦️\n\nനിങ്ങൾ ചോദിക്കുന്ന സിനിമകൾ ലഭിക്കണം എന്നുണ്ടെങ്കിൽ നിങ്ങൾ ഞങ്ങളുടെ ചാനലിൽ ജോയിൻ ചെയ്തിരിക്കണം. ജോയിൻ ചെയ്യാൻ 𝗥𝗲𝗾𝘂𝗲𝘀𝘁 𝘁𝗼 𝗝𝗼𝗶𝗻 ⚓️ എന്ന ബട്ടണിൽ ക്ലിക്ക് ചെയ്യാവുന്നതാണ്.\n\nജോയിൻ ചെയ്ത ശേഷം 🔄 Try Again 🔄 എന്ന ബട്ടണിൽ അമർത്തിയാൽ നിങ്ങൾക്ക് ഞാൻ ആ സിനിമ അയച്ചു തരുന്നതാണ്..\n\nCLICK 𝗥𝗲𝗾𝘂𝗲𝘀𝘁 𝘁𝗼 𝗝𝗼𝗶𝗻 ⚓️ AND THEN CLICK 🔄 Try Again 🔄 BUTTON TO GET MOVIE FILE 🗃️**",
